...
import pandas as pd
import numpy as np
r_wine=pd.read_csv("/home/himanshu/Downloads/winequality-red.csv",delimiter=';')
r_wine[:10]
...
x=np.ones((m,1))
x.shape
r_wine = (r_wine - r_wine.mean())/r_wine.std()
r_wine[:10]
...
y=r_wine[['quality']].values
X=r_wine.drop(labels='quality',axis=1)
...
for i in range(iteration) :
    theta=theta-(alpha/m)*(np.dot(X.T,h-y))        #derivative
    h=X.dot(theta)
